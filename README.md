...
4. **环境变量**（Dashboard → **Workers & Pages** → 项目 → **Settings** → **Variables**）  
   | 变量 | 说明 |
   |------|------|
   | **`STRIPE_SECRET_KEY`** | 未设置 `MAISON_HAN_API_BASE` 时**必填**，否则接口返回 500。 |
   | **`MAISON_HAN_API_BASE`** | （可选）TaTa **HTTPS + context-path**，无尾斜杠，例如 **`https://api.example.com/prod-api`**（与 `.env.production` 里 `VITE_APP_BASE_API=/prod-api` 对应）。只写域名不写 `/prod-api` 时，`/storefront/products` 常 **404**。 |
   | **`PRODUCT_IMAGE_BASE`** | （可选）商品图为 `/profile/...` 时，填与浏览器可访问的图片同源前缀（常为 TaTa 域名），便于拼成绝对 URL。 |
